...
where $G_f = 8.0$ mmHg/L is the fluid MAP gain (calibrated against the CLOVERS trial \citep{shapiro2023clovers}), $V$ is intravascular volume excess, and $d_v$ is vasopressor dose. The fluid saturation term $1/(1+0.3V)$ models the Frank-Starling plateau. Volume dynamics include redistribution at rate 0.15/h.
...
A simplified 3-component Sequential Organ Failure Assessment (SOFA) score \citep{vincent1996sofa} uses cardiovascular (MAP and vasopressor dose), renal (urine output), and metabolic (lactate) components, each scored 0--4 for a total range of 0--12.
...
\begin{itemize}
    ...
    \item \textbf{Heuristic}: Implements SSC guidelines---750 mL fluid if MAP $< 65$, graded response for higher MAP, volume-aware cutoff at 3.5L. For the management environment, gives antibiotics immediately and escalates vasopressors for refractory hypotension.
...
All experiments use seed 42 for reproducibility. PPO hyperparameters are summarized in Table~\ref{tab:hyperparams}. Evaluation uses 50 episodes with deterministic policy.
...
\paragraph{FluidResuscitation-v0.} In this environment, antibiotics dominate patient outcomes---once the infection is suppressed, MAP recovers with minimal fluid intervention. Both random and heuristic agents achieve similar high rewards because the environment provides antibiotics by default, making fluid dosing a secondary factor. PPO's negative reward indicates that with 200K timesteps, the policy overfits to aggressive fluid administration, incurring overload penalties. This highlights the environment's value as a benchmark: naive reward maximization does not suffice when penalty terms are present. Longer training horizons or curriculum strategies may resolve this.

\paragraph{VasopressorTitration-v0.} PPO achieves the best performance ($-36.9 \pm 0.9$), outperforming both random ($-41.3$) and heuristic ($-41.0$) baselines with substantially lower variance. The learned policy applies conservative dose increments that maintain MAP in the target range without accumulating ischemia penalties.

\paragraph{SepsisManagement-v0.} The multi-action environment shows the clearest separation between agent quality. Random actions produce high variance ($\sigma = 34.5$) with frequent early termination (mean episode length 64.2 vs.\ 72.0 maximum), indicating inconsistent patient survival. Both heuristic ($61.2 \pm 2.3$) and PPO ($59.8 \pm 1.8$) achieve full episode survival with low variance, demonstrating that the environment rewards coordinated antibiotic timing, fluid, and vasopressor management. PPO's slightly lower mean than the heuristic suggests that with 500K timesteps the policy has not yet fully converged; the heuristic encodes clinical guidelines that are near-optimal for this reward structure.
